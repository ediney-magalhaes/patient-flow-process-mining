...
| `internacao_clinica_direta` | Bloco B | Sem consulta prévia na emergência, sem cirurgia vinculada |
| `cirurgia_ambulatorial` | — (regra já existente) | Cirurgia sem internação vinculada (`LEFT ANTI JOIN` contra `silver_internacoes`) |

**Emenda (2026-08-13):** a hipótese original deste ADR — de que "internação clínica sem emergência e sem cirurgia" não existiria na prática clínica do hospital — foi **refutada por investigação de dado real** durante a validação do Bloco B (RQ-010). 58 dos 422 registros do Bloco B (~14%) caíram na branch de investigação temporária (`journey_type` provisório, não persistido em produção). A investigação descartou gap de curadoria (nenhum dos 58 casos tinha registro de emergência do mesmo paciente próximo no tempo — 53 sem nenhuma emergência associada, 5 com emergências posteriores e não relacionadas) e confirmou que se trata de um padrão operacional real: **internação clínica direta**, sem passagem por emergência nem cirurgia. O nome de negócio `internacao_clinica_direta` foi definido pelo usuário e incorporado como sexta categoria oficial, substituindo o rótulo temporário de investigação. Ver RQ-010 em `data-quality-rules.md` para o detalhamento completo da investigação.

Nenhuma regra de tratamento automático foi definida a priori para casos fora das categorias esperadas — qualquer registro fora do conjunto conhecido é investigado e nomeado no momento em que aparece durante a construção e validação do Bloco B, não tratado retroativamente nem via mecanismo de quarentena automatizado. Essa decisão é deliberadamente escopada ao volume e à maturidade atuais do projeto (uma base de teste, carga mensal manual); se o padrão se tornar recorrente em produção, um ADR específico tratará da automação do tratamento de exceção.
...
- O caso antes tratado como hipotético (`internacao_clinica_direta`) confirmou-se real e com volume não desprezível (~14% do Bloco B) já na primeira validação. Não há tratamento automatizado para categorias futuras ainda não mapeadas — se um sétimo padrão aparecer, será investigado e nomeado da mesma forma, pontualmente.
